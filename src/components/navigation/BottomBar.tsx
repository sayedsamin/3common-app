...
export function BottomBar({ state, navigation, insets }: BottomTabBarProps) {
  return <View className="bg-surface" style={{ paddingBottom: insets.bottom, paddingLeft: insets.left, paddingRight: insets.right }}>
    <View accessibilityRole="tablist" className="min-h-14 flex-row gap-1 px-2 py-1">
      {state.routes.map((route, index) => {
        ...
        return <Pressable key={route.key} accessibilityRole="tab" accessibilityLabel={item.label} accessibilityState={{ selected }} aria-selected={selected}
          ...
          className="min-h-12 min-w-12 flex-1 items-center justify-center gap-1 rounded-control px-1 py-1 active:bg-surface-muted web:focus-visible:outline-2 web:focus-visible:outline-focus">
          <Icon name={item.icon} tone={selected ? 'success' : 'muted'} size={22} />
          <Text variant="caption" className={cn('text-center text-muted', selected && 'text-success')}>{item.label}</Text>
        </Pressable>;
      })}
    </View>
  </View>;
