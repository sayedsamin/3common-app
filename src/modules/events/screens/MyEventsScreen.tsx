...
import { eventFilterError } from '../filter-schemas';
    ...
    <FlashList data={data?.data ?? []} renderItem={renderEvent} keyExtractor={item => item.id}
      ...
      ListHeaderComponent={<View className="gap-3 pb-4">
        <ListToolbar value={controls} onChange={setControls} primaryOptions={statusOptions} sortOptions={sortOptions} searchPlaceholder="Search events"
          renderExtraFilters={(draft, onChange) => <EventFilters draft={draft} onChange={onChange} />}
          validateFilters={draft => eventFilterError(draft.filters)}
          extraFilterLabels={{ startAfter: `From ${controls.filters.startAfter ?? ''}`, startBefore: `Through ${controls.filters.startBefore ?? ''}`, advanced: 'Advanced conditions' }} />
      ...
      ListEmptyComponent={!isBusy && !query.error && query.fetchStatus !== 'paused'
        ? <EmptyState title="No events found" description={controls.search || controls.primaryFilter !== 'all' || Object.values(controls.filters).some(Boolean) ? 'Try another search or adjust your filters.' : 'Your events will appear here when they are available.'} /> : null}
